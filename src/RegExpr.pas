...
{$IFNDEF FPC} // the option is not supported in FreePascal
  {$IFDEF MSWINDOWS} // not supported in Linux
...
function WideCharUpperCase(AChar : WideChar) : WideChar;
begin
  case Ord(AChar) of
    // Basic Latin
...
end;  { of function WideCharUpperCase
--------------------------------------------------------------}

function WideCharLowerCase(AChar : WideChar) : WideChar;
begin
  case Ord(AChar) of
    // Basic Latin
...
end;  { of function WideCharLowerCase
--------------------------------------------------------------}

class function TRegExpr.InvertCaseFunction (const Ch : REChar) : REChar;
begin
  Result := {$IFDEF FPC}AnsiUpperCase (Ch) [1]{$ELSE} REChar ( WideCharUpperCase(Ch) ){$ENDIF};
  if Result = Ch then
    Result := {$IFDEF FPC}AnsiLowerCase (Ch) [1]{$ELSE} REChar ( WideCharLowerCase(Ch) ){$ENDIF};
